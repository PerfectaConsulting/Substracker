...
function initializeDashboard() {
  document.body.innerHTML = getShellHTML();
  setupNavigation();
  showWelcome();
}
  ...
  list.innerHTML =
    arr.length === 0
      ...
      : arr
          .map(
            (d) => `
        <button class="payment-card" data-action="edit-dept" data-code="${escapeHtml(
          d.code || ""
        )}">
    ...
    list.innerHTML =
      arr.length === 0
        ...
        : arr
            .map(
              (c) => `
          <button class="payment-card" data-action="edit-cat" data-code="${escapeHtml(
            c.code || ""
          )}">
  ...
  if (dd) {
    ...
    dd.innerHTML =
      ...
      arr
        ...
        .join("");
    if ([...dd.options].some((o) => o.value === sel)) dd.value = sel;
  }
}
...
function displayInitialSetup(setupData) {
  ...
  // Delegated click handler scoped to this view container (NO { once: true })
